...
from diagrams.aws.management import (
    Organizations, OrganizationsAccount, ControlTower,
)
...
def create_aws_org_diagram():
    ...
    with Diagram(
        ...
        with Cluster("Application OU"):
            iqa_acct = OrganizationsAccount(
                "IQA Account\n[10.10.0.0/16]\nSpoke — EKS / RDS / S3"
            )
            sb_acct = OrganizationsAccount(
                "Sandbox Account\n[10.20.0.0/16]\nSpoke — EKS / RDS / S3"
            )
            iqa_iam = IAM("IAM\n(IQA-scoped roles)")
        ...
        tgw >> Edge(style="dashed", color="purple", label="RAM share") >> iqa_acct
        tgw >> Edge(style="dashed", color="purple", label="RAM share") >> sb_acct
        tgw >> Edge(style="dashed", color="purple", label="RAM share") >> obs_acct

        # ── Cross-account IAM trust ───────────────────────────────────────
        obs_iam >> Edge(style="dotted", color="goldenrod", label="cross-account\nIAM trust") >> iqa_iam
        obs_iam >> Edge(style="dotted", color="goldenrod", label="cross-account\nIAM trust") >> sb_iam
